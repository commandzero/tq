...
This historical migration report covers 80 JSON-family files under the
repository-root `tests/` directory converted to TOON. Only those 80 converted
files enter these historical totals. The current fixture inventory is separate
and is not interchangeable with this migration snapshot. It contains seven
number-envelope records in `tests/compatibility/baselines/jq-yq-mvp-v1.toon`;
that current count does not replace the historical migration count below.

The retained conversions and ledger model corrections in this historical
migration snapshot save **163,266 o200k_base tokens, or 18.41%**,
against the original files as stored, not a regenerated compact-JSON baseline.

## Historical migration totals

| Storage change | Files | Original o200k_base tokens | TOON o200k_base tokens at migration | Tokens saved | Saved % |
| --- | ---: | ---: | ---: | ---: | ---: |
| JSON to TOON, including ledger model corrections | 80 | 886,759 | 723,493 | 163,266 | 18.41% |

| Tokenizer, converted JSON documents only | Original tokens | TOON tokens at migration | Tokens saved | Saved % |
| --- | ---: | ---: | ---: | ---: |
...
The per-file table labels each baseline format. The historical totals include
the subsequent manual-ledger model correction. Its effect and a same-model encoding comparison
...
reserved single-field objects for values outside ordinary TOON storage. The
counts below are from the historical migration snapshot, not the current
fixture inventory.

| Stored record | Meaning | Occurrences |
| --- | --- | ---: |
| `$tq.fixture.number: "<literal>"` | Historical migration record; restore the exact numeric literal, including values outside tq's numeric envelope | 103 |
...
The historical migration used exact originals from an ignored local backup.
That backup is not part of a clean checkout and may disappear when the build
directory is cleaned. JSON originals no longer exist under `tests/`, so the
original-side measurements cannot be reproduced from this checkout. The
historical totals and per-file counts remain unchanged. A regression test checks
...
An earlier migration-era Clippy run exited successfully while emitting a
pre-existing unknown-lint warning for `clippy::assert_is_empty`; that allowance
has since been removed. An earlier `okf validate docs` run also reported missing
metadata on eight existing documents. The current full documentation bundle
validates 127 concepts with 0 errors, 0 warnings, and 8 informational link
notices.

To reproduce a file pair's counts, supply an available copy of the original
JSON and the committed TOON path. The first argument below is a placeholder;
this checkout does not provide the deleted original:
...
These historical migration measurements compare **original JSON as stored**, not
uniformly compact JSON, with the TOON files at migration time. The **Baseline format** column identifies
...
replacement TOON file where it remains a source fixture. Entries marked
"archived execution record" retain the original measurements, but their full
process dumps are no longer committed. These are historical migration totals,
not the current fixture inventory or measurements of later report revisions.
...
| `coverage-v1.json`, archived execution record | Formatted JSON | 216,909 | 178,167 | 17.86 | 770,553 | 584,084 |
...
| `manual-comparison.json`, archived execution record | Formatted JSON | 320,112 | 262,515 | 17.99 | 1,055,087 | 812,434 |
...
| `manual-execution.json`, archived execution record | Formatted JSON | 97,165 | 75,602 | 22.19 | 358,100 | 270,958 |
