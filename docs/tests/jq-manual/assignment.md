...
During the initial source audit, jq passed all six table queries. The tq development build then reported assignment-path errors for the recursive boolean update at table line 54 and the selected-post comment update at fence line 144. These are historical observations, superseded by the generated Results below. Both cases remain enabled.
