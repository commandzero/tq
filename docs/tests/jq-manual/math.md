---
type: Report
title: "jq manual math coverage"
description: "Recorded review of jq manual math coverage."
generated: { by: codex/gpt-5.6-luna, at: 2026-09-13T23:02:50Z }
---

# jq manual math coverage

This page preserves a historical review snapshot. The counts, observations, and
token totals below describe that snapshot and are not current acceptance
evidence. Regenerate the comparison report before making a claim about a later
build.

Source: [pinned jq manual source inventory](../../../tests/compatibility/reviews/jq-manual/source-examples.toon). The canonical ledger is [math.toon](../../../tests/compatibility/reviews/jq-manual/math.toon).

The source has prose and signature lists only. It has no tables or fenced examples.

The corrected ledger contains 62 executable examples and 6 coverage notes. This
page displays 64 cases because `completeness.toon` assigns two additional audit
witnesses, `manual.audit.math.integer-scale-boundary` and
`manual.audit.math.scalb-infinite-boundary`, to this collection for `scalbln/2`
and `scalb/2` signature coverage. Those same witnesses are linked from
[math-boundaries](math-boundaries.md), so the page total is 62 ledger examples
plus 2 linked audit witnesses. Every named function has a case in
[manual-math.jsonl](../../../tests/compatibility/cases/manual-math.jsonl), and
all jq/tq adapters stay enabled so unsupported functions remain visible.

See the [model and storage comparison](math-model.md) for the schema, migration details, and measured character/token savings.

The one-input witnesses use small values in each function's domain. For example,
`acosh` uses 1, `atanh` uses 0, and `sqrt` uses 4. The ledger records every input.

The two-input witnesses are finite, valid, and simple: `atan2(0;1)`, `copysign(2;-1)`, `drem(5;2)`, `fdim(5;2)`, `fmax(2;3)`, `fmin(2;3)`, `fmod(5;2)`, `hypot(3;4)`, `jn(0;0)`, `ldexp(1;3)`, `nextafter(1;2)`, `nexttoward(1;2)`, `pow(2;3)`, `remainder(5;2)`, `scalb(2;3)`, `scalbln(2;3)`, and `yn(0;1)`. `fma(2;3;4)` covers the three-input signature.

There is a target discrepancy. The published two-input list names `frexp` and `modf`, but historical jq 1.8.1 arity probes recorded both as undefined (`frexp/2` and `modf/2`). The current pinned reference identity is documented in the [reference pin](../../../tests/compatibility/reviews/jq-manual/reference-pin.toon). The raw probe report is historical evidence and is not retained in this checkout. Their cases retain the documented query and record a compile error as informative, with the review disposition `reference-unavailable`.

jq's builtin list provides `frexp/0` and `modf/0`. Additional cases test those
arities with inputs 8 and 3.5, respectively. The historical reference
observations recorded `[0.5,4]` and `[0.5,3]`.

<!-- tq-manual-compare:begin section=math -->
## Results

[Case collection](../../../tests/compatibility/reviews/jq-manual/math.toon)


| Verdict | Cases |
| --- | ---: |
| match | 64 |

Independent output campaigns must pass too. Compact JSON compares exact stdout bytes and process behavior; TOON compares ordered values and process behavior with the JSON execution.

| Output campaign | Matches | Cases |
| --- | ---: | ---: |
| compact_json | 64 | 64 |
| toon | 64 | 64 |

A match requires equivalent JSON results and process behavior, or a matching non-JSON CLI contract. Reviewed disparities retain exact observations and count separately from matches. Historical expected-difference labels do not pass either gate.

Missing features and unaccepted mismatches remain failures. Reference discrepancies describe errors in the imported manual, not successful compatibility.

JSON equivalence ignores whitespace and object key order but retains array and result-sequence order. Error-only cases do not count as JSON matches or size samples. Raw CLI cases keep their original arguments and have no JSON/TOON size measurement.

### Output size

62 eligible examples. Counts use the `o200k_base` and `cl100k_base` tokenizers over complete stdout, including trailing newlines. The totals compare default `-o json` output with default LF-terminated `-o toon` results; TOON sequence captures, when available, are shown in the cases but excluded from size totals. Diff is TOON tokens minus JSON tokens. % is the signed percent difference `(TOON - JSON) / JSON`, so savings are negative and growth is positive.

| Tokenizer | JSON tokens | TOON tokens | Diff | % |
| --- | ---: | ---: | ---: | ---: |
| `o200k_base` | 247 | 229 | -18 | -7.29% |
| `cl100k_base` | 247 | 229 | -18 | -7.29% |

Only successful jq/JSON/TOON-equivalent results enter the totals. A negative `Diff` means TOON uses fewer tokens; `%` is negative for savings and positive for growth. The manual is a correctness corpus, not a representative workload benchmark.

### Reviewed disparities and historical differences

| Case | Reason |
| --- | --- |

### Cases

Each case shows the original jq invocation and the available jq, tq JSON, and tq TOON output captures. Missing captures are marked `<not run>`; these placeholders are not execution evidence. Control bytes use `\xNN` escapes so record separators remain visible. This section is generated by the separate `tq-manual-compare` command.

#### manual.audit.math.integer-scale-boundary

```
# input
jq '[ldexp(2;2147483648),ldexp(2;2147483647),ldexp(2;-2147483649),ldexp(2;nan),scalbln(2;nan)]'

# jq
[
  1.7976931348623157e+308,
  1.7976931348623157e+308,
  0,
  2,
  2
]

# tq -o json
[
  1.7976931348623157e+308,
  1.7976931348623157e+308,
  0,
  2,
  2
]

# tq
[5]: 1.7976931348623157e+308,1.7976931348623157e+308,0,2,2
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 42 | 42 | 34 | -8 | -19.05% |
| `cl100k_base` | 42 | 42 | 34 | -8 | -19.05% |

#### manual.audit.math.scalb-infinite-boundary

```
# input
jq '[scalb(0; infinite), scalb(0; -infinite), scalb(infinite; -infinite), scalb(2; 0.5)]'

# jq
[
  0,
  0,
  1.7976931348623157e+308,
  2
]

# tq -o json
[
  0,
  0,
  1.7976931348623157e+308,
  2
]

# tq
[4]: 0,0,1.7976931348623157e+308,2
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 28 | 28 | 22 | -6 | -21.43% |
| `cl100k_base` | 28 | 28 | 22 | -6 | -21.43% |

#### manual.math.acos

```
# input
jq acos

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.acosh

```
# input
jq acosh

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.asin

```
# input
jq asin

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.asinh

```
# input
jq asinh

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.atan

```
# input
jq atan

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.atan2

```
# input
jq 'atan2(0;1)'

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.atanh

```
# input
jq atanh

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.cbrt

```
# input
jq cbrt

# jq
2

# tq -o json
2

# tq
2
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.ceil

```
# input
jq ceil

# jq
2

# tq -o json
2

# tq
2
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.copysign

```
# input
jq 'copysign(2;-1)'

# jq
-2

# tq -o json
-2

# tq
-2
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 3 | 3 | 3 | 0 | +0% |
| `cl100k_base` | 3 | 3 | 3 | 0 | +0% |

#### manual.math.cos

```
# input
jq cos

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.cosh

```
# input
jq cosh

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.drem

```
# input
jq 'drem(5;2)'

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.erf

```
# input
jq erf

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.erfc

```
# input
jq erfc

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.exp

```
# input
jq exp

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.exp10

```
# input
jq exp10

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.exp2

```
# input
jq exp2

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.expm1

```
# input
jq expm1

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.fabs

```
# input
jq fabs

# jq
3

# tq -o json
3

# tq
3
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.fdim

```
# input
jq 'fdim(5;2)'

# jq
3

# tq -o json
3

# tq
3
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.floor

```
# input
jq floor

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.fma

```
# input
jq 'fma(2;3;4)'

# jq
10

# tq -o json
10

# tq
10
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.fmax

```
# input
jq 'fmax(2;3)'

# jq
3

# tq -o json
3

# tq
3
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.fmin

```
# input
jq 'fmin(2;3)'

# jq
2

# tq -o json
2

# tq
2
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.fmod

```
# input
jq 'fmod(5;2)'

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.frexp-actual-arity

```
# input
jq frexp

# jq
[
  0.5,
  4
]

# tq -o json
[
  0.5,
  4
]

# tq
[2]: 0.5,4
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 12 | 12 | 10 | -2 | -16.67% |
| `cl100k_base` | 12 | 12 | 10 | -2 | -16.67% |

#### manual.math.frexp-reference-unavailable

```
# input
jq 'frexp(8;0)'

# jq


[stderr]
jq: error: frexp/2 is not defined at <top-level>, line 1, column 1:
    frexp(8;0)
    ^^^^^
jq: 1 compile error

# tq -o json


[stderr]
tq: query compilation failed: TQ-RESOLVE-ARITY-001: invalid arity 2 for built-in frexp

# tq


[stderr]
tq: query compilation failed: TQ-RESOLVE-ARITY-001: invalid arity 2 for built-in frexp
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 0 | 0 | 0 | 0 | n/a |
| `cl100k_base` | 0 | 0 | 0 | 0 | n/a |

#### manual.math.gamma

```
# input
jq gamma

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.hypot

```
# input
jq 'hypot(3;4)'

# jq
5

# tq -o json
5

# tq
5
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.j0

```
# input
jq j0

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.j1

```
# input
jq j1

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.jn

```
# input
jq 'jn(0;0)'

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.ldexp

```
# input
jq 'ldexp(1;3)'

# jq
8

# tq -o json
8

# tq
8
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.lgamma

```
# input
jq lgamma

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.log

```
# input
jq log

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.log10

```
# input
jq log10

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.log1p

```
# input
jq log1p

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.log2

```
# input
jq log2

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.logb

```
# input
jq logb

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.modf-actual-arity

```
# input
jq modf

# jq
[
  0.5,
  3
]

# tq -o json
[
  0.5,
  3
]

# tq
[2]: 0.5,3
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 12 | 12 | 10 | -2 | -16.67% |
| `cl100k_base` | 12 | 12 | 10 | -2 | -16.67% |

#### manual.math.modf-reference-unavailable

```
# input
jq 'modf(3;0)'

# jq


[stderr]
jq: error: modf/2 is not defined at <top-level>, line 1, column 1:
    modf(3;0)
    ^^^^
jq: 1 compile error

# tq -o json


[stderr]
tq: query compilation failed: TQ-RESOLVE-ARITY-001: invalid arity 2 for built-in modf

# tq


[stderr]
tq: query compilation failed: TQ-RESOLVE-ARITY-001: invalid arity 2 for built-in modf
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 0 | 0 | 0 | 0 | n/a |
| `cl100k_base` | 0 | 0 | 0 | 0 | n/a |

#### manual.math.nearbyint

```
# input
jq nearbyint

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.nextafter

```
# input
jq 'nextafter(1;2)'

# jq
1.0000000000000002

# tq -o json
1.0000000000000002

# tq
1.0000000000000002
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 9 | 9 | 9 | 0 | +0% |
| `cl100k_base` | 9 | 9 | 9 | 0 | +0% |

#### manual.math.nexttoward

```
# input
jq 'nexttoward(1;2)'

# jq
1.0000000000000002

# tq -o json
1.0000000000000002

# tq
1.0000000000000002
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 9 | 9 | 9 | 0 | +0% |
| `cl100k_base` | 9 | 9 | 9 | 0 | +0% |

#### manual.math.pow

```
# input
jq 'pow(2;3)'

# jq
8

# tq -o json
8

# tq
8
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.remainder

```
# input
jq 'remainder(5;2)'

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.rint

```
# input
jq rint

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.round

```
# input
jq round

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.scalb

```
# input
jq 'scalb(2;3)'

# jq
16

# tq -o json
16

# tq
16
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.scalbln

```
# input
jq 'scalbln(2;3)'

# jq
16

# tq -o json
16

# tq
16
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.significand

```
# input
jq significand

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.sin

```
# input
jq sin

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.sinh

```
# input
jq sinh

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.sqrt

```
# input
jq sqrt

# jq
2

# tq -o json
2

# tq
2
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.tan

```
# input
jq tan

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.tanh

```
# input
jq tanh

# jq
0

# tq -o json
0

# tq
0
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.tgamma

```
# input
jq tgamma

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.trunc

```
# input
jq trunc

# jq
1

# tq -o json
1

# tq
1
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 2 | 2 | 2 | 0 | +0% |
| `cl100k_base` | 2 | 2 | 2 | 0 | +0% |

#### manual.math.y0

```
# input
jq y0

# jq
0.08825696421567697

# tq -o json
0.08825696421567697

# tq
0.08825696421567697
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 9 | 9 | 9 | 0 | +0% |
| `cl100k_base` | 9 | 9 | 9 | 0 | +0% |

#### manual.math.y1

```
# input
jq y1

# jq
-0.7812128213002887

# tq -o json
-0.7812128213002887

# tq
-0.7812128213002887
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 10 | 10 | 10 | 0 | +0% |
| `cl100k_base` | 10 | 10 | 10 | 0 | +0% |

#### manual.math.yn

```
# input
jq 'yn(0;1)'

# jq
0.08825696421567697

# tq -o json
0.08825696421567697

# tq
0.08825696421567697
```

| Tokenizer | jq | tq -o json | tq | Diff | % |
| --- | ---: | ---: | ---: | ---: | ---: |
| `o200k_base` | 9 | 9 | 9 | 0 | +0% |
| `cl100k_base` | 9 | 9 | 9 | 0 | +0% |

<!-- tq-manual-compare:end -->
