...
The TOON inventory records their inputs and source locations. The Results
section also includes `manual.audit.regex.gsub-three-arguments`, assigned from
the [regex boundary inventory](../../../tests/compatibility/reviews/jq-manual/regex-boundaries.toon).
That linked witness brings the section total from 27 local cases to 28.
...
The inventory records source coverage, not compatibility. The generated Results below record the subsequent campaign, including any remaining failures.
