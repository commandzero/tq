...
The initial source audit used jq 1.8.1 because Apple's jq 1.7.1 did not provide `have_decnum`. The cases kept `tq` enabled so differences would be observable rather than hidden by an adapter skip. The observations below describe that earlier build and 50-case inventory, not the latest campaign in Results.

In that audit, jq completed 50/50 cases, yq 4.53.2 completed 45/46 enabled cases, and the development build of tq completed 43/50. The seven tq errors involved a large exponent, four `have_decnum` queries, `.[4,2]`, and `.\"foo$\"`. Those historical failures are superseded by the generated Results below, which cover the current 51-case collection.
