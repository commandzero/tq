...
The tables and claims below preserve that baseline for provenance only. They do
...
All 251 published input/output examples have exact, runnable jq/tq cases. The 13
published manual sections and the separate `introduction` review account for 14
section-result rows and 68 fenced snippets. The suite adds 517 cases.
...
The original execution snapshot recorded 518 unique referenced cases: 303 matches, 15 historical expected differences, 198 compatibility failures, and 2 reference discrepancies. These are baseline classifications, not current approvals or passing totals. The reference pin and gap inventory retain these classifications; full historical process dumps are not source fixtures.
...
3. The historical full campaign ran 784 catalog cases with 0 harness errors. Its portable report recorded executable hashes and the catalog hash; the complete local observations are not retained in this checkout.
