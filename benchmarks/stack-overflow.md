...
- Mean per-scenario median time was **75.023 ms for jq**, **76.937 ms for yq** (+2.6%) and **74.173 ms for tq** (-1.1%).
- `tq` was faster than `jq` in **31/50** scenarios.
- Mean peak RSS was **2.05 MiB for jq**, **2.70 MiB for yq** (+31.5%) and **2.00 MiB for tq** (-2.4%).
- These are small, single-document inputs, so process startup and fixed runtime overhead dominate; this is a compatibility-shaped smoke benchmark, not a large-data throughput claim.
