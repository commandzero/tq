...
A deadline limits how long the caller waits for a measurement. It does not
permit dropping an unreaped child. If bounded cleanup cannot finish, the
measurement fails and a reaper retains the exact child until its native wait
completes. A pending cleanup blocks further samples instead of accumulating
unbounded background work. Late cleanup cannot turn that failed measurement
into an accepted timing or RSS sample.

Each coordinator permits up to 16 overlapping worker measurements. It reserves
cleanup capacity before launch and rejects new work when that capacity is full
or a previous cleanup is still pending.

The isolated worker is also the target's reaper process. Cancellation and
coordinator loss must leave it alive long enough to terminate and reap its
target. The coordinator must not kill the only remaining target parent merely
because the measurement deadline expired. Reaper shutdown follows completed
cleanup, not publication of an error reply.

This contract does not recover exact-child accounting after an external actor
forcibly kills the reaper itself, or after host failure. Such a loss invalidates
the measurement. All public measurement entry points use the isolated worker;
the direct in-process path exists only for internal validation.
...
is required only when selected for diagnostic limits. The platform `time`
commands are independent validation tools, not the production collectors.
If preflight or a later sample lacks RSS, the harness aborts and the partial
report is not acceptance evidence. Run with the required native child-accounting
permissions. These collection checks do not replace the publication review.
...
The campaign script reads Cargo's emitted executable paths, including configured
target triples and custom target directories. It uses a host-native `tq` from
`PATH` to parse Cargo output and corpus manifests. Set `TQ_HOST_TQ` to select
that parser explicitly; it is separate from the measured `TQ_BIN`. Install a
