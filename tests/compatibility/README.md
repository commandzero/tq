...
compatibility verdicts, complete per-example outputs, and tokenizer counts. The [TOON evidence](reviews/jq-manual/comparison.toon)
...
TQ_JQ=target/reference-build/jq/jq TQ_BIN=target/debug/tq \
  target/debug/tq-manual-compare \
    ...
    tests/compatibility/reviews/jq-manual/comparison.toon | tq -x
```
...
- tq structured output defaults to LF-terminated TOON values. Use `--seq` for
  explicit record boundaries, `--unframed` for exactly one document, and
...
The historical evidence is in [coverage-v1.toon](reviews/coverage-v1.toon). The
