...
impl Number {
    ...
    /// Returns the integral binary64 projection inside the exact index envelope.
    ///
    /// Like jq's index conversion, this operates on the runtime projection, not
    /// the retained decimal used for ordering and identity. Underflowed positive
    /// and negative literals therefore select zero rather than being rejected.
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn index_projection_preserves_reference_underflow_behavior() {
        let zero = Number::parse("0").unwrap();
        for literal in ["1e-400", "-1e-400", "2e-400", "-2e-400"] {
            let value = Number::parse(literal).unwrap();
            assert_ne!(value, zero);
            assert_eq!(value.exact_index(), Some(0));
        }
        assert_eq!(Number::parse("0.1").unwrap().exact_index(), None);
    }
