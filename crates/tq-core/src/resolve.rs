...
use crate::{
    ...
    JsonInputError, JsonInputOptions, ModuleInfo, Number, Object, OptimizerRewrite, Parsed,
    PlanKind, Query, Resolved, SourceFile, SourceId, Span, Value,
    ast::{
...
impl ModuleLoader {
    ...
    fn new_with_source(
        ...
        let roots = options
            .module_roots
            .iter()
            // An empty `-L` argument is the CLI's explicit-library-path
            // sentinel. It is not a filesystem root and must not trigger the
            // invalid-root diagnostic during resolver setup.
    ...
    fn load(
        ...
        self.sources
            .insert(source_id, SourceMetadata::new(source_name.clone(), text));
