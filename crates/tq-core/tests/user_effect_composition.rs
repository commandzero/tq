...
#[test]
fn recoverable_provider_input_errors_are_catchable_and_resume_for_input_builtins() {
    for query in ["[try input catch ., input]", "[try inputs catch ., inputs]"] {
        let plan = analyze(
            resolve(
                parse(query).expect("recoverable input query parses"),
                &ResolveOptions::default(),
            )
            .expect("recoverable input query resolves"),
        )
        .compile()
        .expect("recoverable input query compiles")
        .document_plan();
        let mut responses = vec![
            Err(VmError::RecoverableInput {
                message: Arc::from("recoverable parse failure"),
                context: Arc::from("in 'provider.json' at recovery segment 2"),
            }),
            Ok(Some(InputValue {
                value: number(7),
                identity: Arc::from("provider.json"),
        ...
        let cursor = InputCursor::from_provider(move || {
            responses
                .next()
                .expect("test provider should only be queried three times")
        });
        let mut vm = Vm::new(&plan, Value::Null, VmLimits::default()).with_input_cursor(cursor);

        assert_eq!(
            vm.next_result().expect("recoverable input is caught"),
            Some(Value::array([
                Value::string("recoverable parse failure"),
                number(7),
            ])),
            "{query}"
        );
        assert_eq!(
            vm.next_result().expect("recoverable input query completes"),
            None,
            "{query}"
        );
    }
...
#[test]
fn input_filename_requires_platform_capability_even_with_cursor_context() {
    let query = "def inspect: input | input_filename; inspect";
    let plan = analyze(
        resolve(
...
#[test]
fn input_line_number_remains_capability_free_after_input() {
    let plan = analyze(
        resolve(
            parse("def inspect: input | input_line_number; inspect")
                .expect("input line query parses"),
            &ResolveOptions::default(),
        )
        .expect("input line query resolves"),
    )
    .compile()
    .expect("input line query compiles")
    .document_plan();
    let cursor = InputCursor::from_input_values(vec![InputValue {
        value: number(2),
        identity: Arc::from("provider.json"),
    ...
    assert_eq!(
        vm.next_result().expect("input line evaluates"),
        Some(number(20))
    );
}
