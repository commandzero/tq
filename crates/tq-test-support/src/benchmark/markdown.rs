...
fn adapter_columns<'a>(rows: &[&'a BenchmarkRow]) -> Vec<AdapterColumn<'a>> {
    ...
    columns.sort_by_key(|(adapter, format)| {
        ...
        let format = match format {
            ...
            super::InputFormat::JsonSequence => 3,
            super::InputFormat::Csv => 4,
            super::InputFormat::Tsv => 5,
        };
        (tool, format, *adapter)
    });
    columns.dedup();
...
fn format_name(format: super::InputFormat) -> &'static str {
    match format {
        ...
        super::InputFormat::JsonSequence => "json-seq",
        super::InputFormat::Csv => "csv",
        super::InputFormat::Tsv => "tsv",
    }
}
...
mod tests {
    ...
    #[test]
    fn native_format_columns_have_distinct_labels_and_stable_order() {
        let formats = [
        ...
        let rows = formats
            ...
            .map(|(format, name)| {
                ...
                let mut row = row("benchmark.native-formats", BenchmarkOutcome::Timed);
                row.input_format = *format;
                row.adapter_id = format!("tq-{name}");
                row
            })
            .collect::<Vec<_>>();
        let references = rows.iter().collect::<Vec<_>>();
        let columns = super::adapter_columns(&references);
        assert_eq!(
            columns
                .iter()
                .map(|(_, format)| *format)
                .collect::<Vec<_>>(),
            formats.map(|(format, _)| format)
        );
    }
    ...
    fn rendering_preserves_authored_text_and_is_idempotent() {
        ...
        assert!(first.contains("Profile: `standard` | Campaign status: `passed`"));
        assert!(!first.contains("Profile: `standard` | Status:"));
