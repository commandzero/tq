...
fn report_comparability(
    ...
    let campaign_identity_matches = reasons.is_empty();
    let rows_match = row_identities_match(left, right);
    if !rows_match && campaign_identity_matches {
        reasons.push("benchmark row identities differ".to_owned());
    }
    if rows_match && !measurement_contracts_match(left, right) {
...
fn measurement_contracts_match(
    ...
    left.cases.iter().all(|left_row| {
        right
            ...
            .is_some_and(|right_row| row_measurement_contracts_match(left_row, right_row))
    })
}

fn row_identities_match(left: &BenchmarkCampaignReport, right: &BenchmarkCampaignReport) -> bool {
    left.cases.len() == right.cases.len()
        && left.cases.iter().all(|left_row| {
            left.cases
                .iter()
                .filter(|row| same_row_identity(left_row, row))
                .count()
                == 1
                && right
                    .cases
                    .iter()
                    .filter(|right_row| same_row_identity(left_row, right_row))
                    .count()
                    == 1
        })
        && right.cases.iter().all(|right_row| {
            right
                .cases
                .iter()
                .filter(|row| same_row_identity(right_row, row))
                .count()
                == 1
                && left
                    .cases
                    .iter()
                    .filter(|left_row| same_row_identity(left_row, right_row))
                    .count()
                    == 1
        })
}
...
fn same_row_identity(left: &BenchmarkRow, right: &BenchmarkRow) -> bool {
    left.case_id == right.case_id
        ...
        && left.comparison_families == right.comparison_families
}

fn row_measurement_contracts_match(left: &BenchmarkRow, right: &BenchmarkRow) -> bool {
    sample_measurement_contracts_match(&left.samples, &right.samples)
        && sample_measurement_contracts_match(
            &left.instrumented_samples,
            &right.instrumented_samples,
        )
}

fn sample_measurement_contracts_match(left: &[BenchmarkSample], right: &[BenchmarkSample]) -> bool {
    let left_contracts = sample_measurement_contracts(left);
    let right_contracts = sample_measurement_contracts(right);
    left_contracts.len() == right_contracts.len()
        && left_contracts.iter().all(|contract| {
            left_contracts
                .iter()
...
fn sample_measurement_contracts(
    samples: &[BenchmarkSample],
        ...
        let (unavailable, failures): (Vec<_>, Vec<_>) =
            comparability.reasons.into_iter().partition(|reason| {
                reason == "campaign profile differs"
                    ...
                    || reason == "tool identity differs"
                    || reason == "tq build configuration differs"
                    || reason == "compiler profile differs"
    ...
    command
        ...
        .map(|(index, argument)| {
            if index + 1 == command.len()
                && artifact_path.is_some_and(|path| {
                    !path.is_empty()
                        && Path::new(path).components().all(|component| {
                            !matches!(component, Component::CurDir | Component::ParentDir)
                        })
                        && Path::new(argument).ends_with(path)
                })
            {
                None
            } else {
