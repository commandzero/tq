...
fn worker_is_valid(worker: &WorkerIdentity, collector_source: &str) -> bool {
    !worker.executable_sha256.trim().is_empty()
        && worker.launch_protocol == WORKER_PROTOCOL
...
mod tests {
    ...
    #[test]
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    fn accepted_calibration_matches_a_real_worker_campaign_sample() {
        ...
        let sample_protocol = sample
            .measurement_protocol
            .clone()
            .expect("worker identity on recorded campaign sample");
        let worker = sample_protocol
            .worker
            .clone()
            .expect("worker identity on campaign sample");

        let mut value = summary();
        let collector = worker.collector_source_sha256.clone();
        value["calibration_linkage"]["collector_source_sha256"] = collector.clone().into();
        value["worker_isolation"]["collector_source_sha256"] = collector.clone().into();

        let mut primary_protocol = serde_json::to_value(&sample_protocol).unwrap();
        primary_protocol["isolation_evidence"] =
            value["calibration_linkage"]["measurement_protocol"]["isolation_evidence"].clone();
        value["calibration_linkage"]["measurement_protocol"] = primary_protocol;
        value["worker_control_measurement_protocol"] =
            serde_json::to_value(&sample_protocol).unwrap();

        let bytes = serde_json::to_vec(&value).unwrap();
        let mut calibrations =
            TimingCalibration::from_bytes_with_worker(&bytes, "host", &collector, Some(&worker))
                .expect("accepted worker calibration");
        assert_eq!(calibrations.len(), 1);
        let calibration = calibrations.pop().unwrap();
        assert!(calibration.matches(&sample_protocol));

        let mut calibrated = sample_protocol;
        calibration
            .apply(&mut calibrated)
            .expect("calibration applies to worker sample");
        assert_eq!(calibrated.worker, Some(worker));
    ...
    fn campaign_case() -> BenchmarkCase {
        BenchmarkCase {
            schema_version: 1,
            id: "benchmark.calibration-worker".to_owned(),
            compatibility_gate: "common.identity.empty".to_owned(),
    ...
    fn campaign_corpus() -> BenchmarkCorpusIdentity {
        BenchmarkCorpusIdentity {
            ...
            source_id: "calibration-worker".to_owned(),
    ...
    fn empty_reference() -> CorrectnessObservation {
        CorrectnessObservation {
            payload: CorrectnessPayload::SemanticSequence(
                semantic_digest(&[]).expect("empty semantic digest"),
            ),
            process_status: tq_test_support::compatibility::ProcessStatus::Exited,
            exit_code: Some(0),
            error_class: None,
        }
    }
    ...
    #[test]
    fn previous_worker_protocol_is_rejected_not_relabeled() {
        let mut value = summary();
