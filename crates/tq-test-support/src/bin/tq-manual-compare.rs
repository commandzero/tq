...
mod tests {
    ...
    #[test]
    fn checked_in_sections_match_the_saved_observations() {
        ...
        .unwrap();
        let committed = root.join("docs/tests/jq-manual");
        let output = tempfile::tempdir().unwrap();
        for entry in std::fs::read_dir(&committed).unwrap() {
            let entry = entry.unwrap();
            if entry.path().extension().is_some_and(|ext| ext == "md") {
                std::fs::copy(entry.path(), output.path().join(entry.file_name())).unwrap();
            }
        }
        let authored = std::fs::read_dir(output.path())
            .unwrap()
            .map(|entry| {
                let entry = entry.unwrap();
                let text = std::fs::read_to_string(entry.path()).unwrap();
                let prefix = text
                    .find(super::GENERATED_START)
                    .map_or(text.as_str(), |index| &text[..index])
                    .trim_end()
                    .to_owned();
                (entry.file_name(), prefix)
            })
            .collect::<std::collections::BTreeMap<_, _>>();
        super::write_markdown_sections(
            output.path(),
            &report,
            &root.join("tests/compatibility/reviews/jq-manual"),
        )
        .unwrap();
        for (name, prefix) in &authored {
            let generated_text = std::fs::read_to_string(output.path().join(name)).unwrap();
            assert!(
                generated_text.starts_with(prefix),
                "authored content changed in {name:?}"
            );
        }
        let first = std::fs::read_dir(output.path())
            .unwrap()
            .map(|entry| {
                let entry = entry.unwrap();
        ...
        .unwrap();
        let second = std::fs::read_dir(output.path())
            .unwrap()
            .map(|entry| {
                let entry = entry.unwrap();
                (entry.file_name(), std::fs::read(entry.path()).unwrap())
            ...
            .collect::<std::collections::BTreeMap<_, _>>();
        assert_eq!(first, second, "rendering is not idempotent");
        for (name, bytes) in &first {
            assert_eq!(
                bytes,
                &std::fs::read(committed.join(name)).unwrap(),
                "generated page differs from the checked-in page: {name:?}"
            );
        }
        assert!(!output.path().join("tokens").exists());
        let index = std::fs::read_to_string(output.path().join("index.md")).unwrap();
        assert!(index.contains("1. [Review data]"));
        assert!(index.contains("advanced-features.md"));
        ...
        let generated_pages = first
            .keys()
            .filter(|name| name.as_encoded_bytes().ends_with(b".md"))
            .filter(|name| *name != "index.md")
            .count();
        assert_eq!(generated_pages, 23);
