...
use tq_test_support::{
    benchmark::{
        ...
        BenchmarkRunnerError, BenchmarkSampling, BenchmarkTool, Comparability, ComparisonFamily,
        DatasetFamily, DatasetSelector, DatasetTier, ExecutionClass, InputFormat, OutputContract,
        OutputContractKind, RegressionGate, collect_environment, correctness_witness_limit_row,
        normalize_correctness_run, populate_reference_ratios, preflight_rss, run_gated_row,
        unsupported_row,
    },
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    // RSS preflight invokes this executable as a lightweight control/probe
    // process. Handle those private modes before campaign setup touches the
    // repository, scenarios, or report paths.
    ...
    for record in &scenarios {
        ...
        if reference.as_ref().is_some_and(|reference| {
            reference.process_status != ProcessStatus::Exited || reference.exit_code != Some(0)
        }) {
            return Err(
        ...
        for tool in [BenchmarkTool::Jq, BenchmarkTool::Yq, BenchmarkTool::Tq] {
            ...
            let Some(identity) = tools.get(&tool) else {
                let placeholder = BenchmarkInvocation {
                    ...
                    args: tool_args(
                        ...
                        tool,
                        &record.scenario.benchmark.query,
                    ),
            ...
            let row = if let Some(reference) = &reference {
                ...
                return Err("missing correctness reference".into());
            };
            rows.push(row);
        }
    }
...
fn invocation(
    ...
    let args = tool_args(benchmark.output_mode, adapter.tool, &benchmark.query);
    BenchmarkInvocation {
        cancellation: None,
