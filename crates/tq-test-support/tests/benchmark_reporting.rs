...
#[test]
fn changed_comparison_families_are_not_report_comparable() {
    for adapter_id in ["tq-json", "jq-json"] {
        let mut baseline = campaign("machine-a", "digest-a", 100, 1024);
        baseline.cases[0].adapter_id = adapter_id.to_owned();
        assert!(compare_reports(&baseline, &baseline).comparable);
        let mut candidate = baseline.clone();
        candidate.cases[0].comparison_families = vec![ComparisonFamily::NativeFormat];

        let comparison = compare_reports(&baseline, &candidate);
        assert!(!comparison.comparable, "{adapter_id}: {comparison:?}");
        assert_eq!(comparison.reasons, ["benchmark row identities differ"]);
    }
}
...
#[test]
fn changed_measurement_sample_multiplicity_is_not_report_comparable() {
    let baseline = campaign("machine-a", "digest-a", 100, 1024);
    let mut candidate = baseline.clone();
    candidate.cases[0].samples.pop();
    candidate.cases[0].requested_samples = 2;
...
#[test]
fn primary_and_instrumented_measurement_contracts_remain_separate() {
    let baseline = limited_campaign(100, 1024);
    let mut candidate = baseline.clone();
    candidate.cases[0].samples.pop();
    candidate.cases[0]
        .instrumented_samples
        .push(baseline.cases[0].instrumented_samples[0].clone());
    candidate.cases[0].summary = summarize_samples(&candidate.cases[0].samples, 100, 1);
...
#[test]
fn regression_gate_rejects_missing_added_and_duplicate_row_identities() {
    let mut baseline = campaign("machine-a", "digest-a", 100, 1024);
    let mut second = baseline.cases[0].clone();
    second.case_id = "benchmark.second".to_owned();
    baseline.cases.push(second);
    for mutation in ["missing", "added", "duplicate"] {
        let mut candidate = baseline.clone();
        match mutation {
            "missing" => {
                candidate.cases.pop();
            ...
            "added" => {
                ...
                extra.case_id = "benchmark.extra".to_owned();
                candidate.cases.push(extra);
            }
            _ => {
                candidate.cases[1] = candidate.cases[0].clone();
            }
        }
        assert!(
            !compare_reports(&baseline, &candidate).comparable,
            "{mutation}"
        );
        ...
        assert!(!gate.evaluated, "{mutation}");
        assert!(
            gate.unavailable.is_empty(),
            "{mutation}: {:?}",
            gate.unavailable
        );
        assert!(!gate.failures.is_empty(), "{mutation}");
    }
}

#[test]
fn environment_mismatch_is_unavailable_not_a_regression_failure() {
    let baseline = campaign("machine-a", "digest-a", 100, 1024);
    let mut candidate = baseline.clone();
    candidate.environment.machine_identity = "machine-b".to_owned();
    ...
    assert_eq!(gate.unavailable, ["machine identity differs"]);
}
...
#[test]
fn self_regression_normalizes_absolute_paths_to_relative_artifacts() {
    let mut baseline = campaign("machine-a", "digest-a", 1000, 1000);
    baseline.corpus[0].artifact.path = "sources/source.json".to_owned();
    baseline.cases[0].command = vec![
        "/baseline/build/tq".to_owned(),
        ".".to_owned(),
        "/baseline/cache/sources/source.json".to_owned(),
    ];

    let mut candidate = baseline.clone();
    candidate.cases[0].command[0] = "/candidate/build/tq".to_owned();
    candidate.cases[0].command[2] = "/candidate/cache/sources/source.json".to_owned();
    ...
    assert!(gate.evaluated, "unavailable: {:?}", gate.unavailable);
}

#[test]
fn self_regression_does_not_normalize_a_nonmatching_path_component() {
    let mut baseline = campaign("machine-a", "digest-a", 1000, 1000);
    baseline.corpus[0].artifact.path = "sources/source.json".to_owned();
    baseline.cases[0].command = vec![
        "/baseline/build/tq".to_owned(),
        ".".to_owned(),
        "/baseline/cache/other-sources/source.json".to_owned(),
    ];

    let mut candidate = baseline.clone();
    candidate.cases[0].command[0] = "/candidate/build/tq".to_owned();
    candidate.cases[0].command[2] = "/candidate/cache/other-sources/source.json".to_owned();
...
#[test]
fn self_regression_does_not_normalize_nonfinal_artifact_like_arguments() {
    let mut baseline = campaign("machine-a", "digest-a", 1000, 1000);
    baseline.corpus[0].artifact.path = "sources/source.json".to_owned();
    baseline.cases[0].command = vec![
        "/baseline/build/tq".to_owned(),
        "sources/source.json".to_owned(),
        "/baseline/cache/sources/source.json".to_owned(),
    ];

    let mut candidate = baseline.clone();
    candidate.cases[0].command[0] = "/candidate/build/tq".to_owned();
    candidate.cases[0].command[1] = "/candidate/cache/sources/source.json".to_owned();
    candidate.cases[0].command[2] = "/candidate/cache/sources/source.json".to_owned();
