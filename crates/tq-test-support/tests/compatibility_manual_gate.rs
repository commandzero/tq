...
fn current_report() -> Value {
    tq_test_support::fixture_data::from_toon(include_bytes!(
        "../../../tests/compatibility/reviews/jq-manual/comparison.toon"
    ))
    .expect("current manual comparison report")
}

fn historical_baseline_report() -> Value {
    let historical: Value = tq_test_support::fixture_data::from_toon(include_bytes!(
        "../../../tests/compatibility/reviews/jq-manual/execution.toon"
    ))
    .expect("historical manual execution report");
    let mut report = current_report();
    let current_cases = report["cases"]
        .as_array()
        .expect("current report cases")
        .clone();
    let historical_cases = historical["execution"]
        .as_array()
        .expect("historical execution cases");
    let cases = historical_cases
        .iter()
        .map(|historical_case| {
            let id = historical_case["id"].as_str().expect("historical case ID");
            let mut current_case = current_cases
                .iter()
                .find(|case| case["id"] == id)
                .unwrap_or_else(|| panic!("historical case missing from current report: {id}"))
                .clone();
            current_case["verdict"] = historical_case["verdict"].clone();
            current_case
        })
        .collect();
...
fn passing_report() -> Value {
    ...
    let review_ids =
        read_manual_review_case_ids(&root().join("tests/compatibility/reviews/jq-manual")).unwrap();
    let template = report["cases"]
        .as_array()
        .unwrap()
        .iter()
        .find(|row| row["verdict"] == "match" && row["contract"] == "result-sequence")
        .unwrap()
        .clone();
    ...
    for case in report["cases"].as_array_mut().unwrap() {
        let source = catalog
            .cases
            .iter()
            .find(|source| case["id"] == source.id)
            .unwrap();
        case["case_fingerprint"] = case_fingerprint(source).unwrap().into();
        case["verdict"] = Value::String("match".to_owned());
        case["differences"] = Value::Array(Vec::new());
        if case["contract"] == "result-sequence" {
            case["json_equivalent"] = Value::Bool(true);
        }
        // The identity cases intentionally compare jq's reference observation
        // with a target-specific tq contract. Preserve their real tq payload
        // while normalizing the other synthetic rows from jq.
        let mut tq = if source.expected.tq_contract.is_some() {
            case["tq"].clone()
        } else {
            case["jq"].clone()
        };
        if source.expected.tq_contract == Some(TqContract::Help) {
            let stdout = tq_help_contract_fixture();
            tq["stdout_hex"] = encode_hex(stdout.as_bytes()).into();
            tq["raw_stdout_hex"] = encode_hex(stdout.as_bytes()).into();
        }
        tq["tool"] = Value::String("tq".to_owned());
        case["tq"] = tq;
        case["tq_toon"] = case["tq"].clone();
        case["toon_contract_match"] = true.into();
        case["compact"] = serde_json::json!({
            "exact": true, "jq": case["jq"], "tq": case["tq"], "differences": [],
        });
    }
    report
}

fn tq_help_contract_fixture() -> &'static str {
    concat!(
