...
fn stalled_requested_sampler_is_bounded_and_propagates_failure() {
    ...
    // This is an end-to-end watchdog for the sampler, worker, and cleanup
    // path.  The sampler's own `ps` deadline is 250 ms; this larger budget
    // allows the documented termination and worker cleanup bounds to apply
    // without turning scheduler contention into a false failure.
    assert!(
        elapsed < Duration::from_secs(10),
        "stalled sampler exceeded end-to-end test budget: {elapsed:?}"
    );
    assert!(!output.status.success());
    assert!(
