...
#[test]
fn unframed_empty_object_uses_exact_reference_cardinality() {
    let args = ["--unframed"];
    let run = |name: &str, values: &[serde_json::Value]| {
        let directory = tempfile::tempdir().expect("temporary directory");
        let executable = script(directory.path(), name, ":");
        run_gated_row(
            &case(),
            &tq_adapter(args),
            &corpus(),
    ...
    assert_eq!(run("empty-object", &[json!({})]), BenchmarkOutcome::Timed);
    assert_eq!(run("zero-results", &[]), BenchmarkOutcome::Incorrect);
    assert_eq!(run("scalar", &[json!(1)]), BenchmarkOutcome::Incorrect);
    assert_eq!(
        run("multiple-results", &[json!({}), json!({})]),
        BenchmarkOutcome::Incorrect
    );
}
