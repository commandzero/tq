...
fn stdout_and_stderr_captures_are_independently_bounded() {
    ...
    .expect("stream capture measurement");
    assert_eq!(limited.status, MeasuredStatus::OutputLimit);
    // Either capture reader may observe its stream crossing its per-stream limit first.
    assert!(limited.output_bytes > 1024 || limited.stderr_bytes > 1024);
    let stdout_path = limited.stdout_path.as_deref().expect("saved stdout");
    let stderr_path = limited.stderr_path.as_deref().expect("saved stderr");
    assert!(std::fs::metadata(stdout_path).unwrap().len() <= 1024);
    assert!(std::fs::metadata(stderr_path).unwrap().len() <= 1024);
    std::fs::remove_file(stdout_path).unwrap();
    std::fs::remove_file(stderr_path).unwrap();

    let mut exact = invocation(&["streams", "1024", "1024"], 1024, Duration::from_secs(2));
    exact.retain_output = true;
    let exact = measure_process(&exact).expect("exact-boundary stream capture measurement");
    assert_eq!(exact.status, MeasuredStatus::Exited);
    assert_eq!(exact.output_bytes, 1024);
    assert_eq!(exact.stderr_bytes, 1024);
    let stdout_path = exact.stdout_path.as_deref().expect("retained exact stdout");
    let stderr_path = exact.stderr_path.as_deref().expect("retained exact stderr");
    assert_eq!(std::fs::metadata(stdout_path).unwrap().len(), 1024);
    assert_eq!(std::fs::metadata(stderr_path).unwrap().len(), 1024);
    std::fs::remove_file(stdout_path).unwrap();
    std::fs::remove_file(stderr_path).unwrap();

    let stderr_only = measure_process(&invocation(
        &["streams", "0", "2048"],
        1024,
        Duration::from_secs(2),
    ))
    .expect("stderr-only stream capture measurement");
    assert_eq!(stderr_only.status, MeasuredStatus::OutputLimit);
    assert_eq!(stderr_only.output_bytes, 0);
    assert!(stderr_only.stderr_bytes > 1024);
    let stdout_path = stderr_only.stdout_path.as_deref().expect("saved stdout");
    let stderr_path = stderr_only.stderr_path.as_deref().expect("saved stderr");
    assert_eq!(std::fs::metadata(stdout_path).unwrap().len(), 0);
    assert!(std::fs::metadata(stderr_path).unwrap().len() <= 1024);
    std::fs::remove_file(stdout_path).unwrap();
    std::fs::remove_file(stderr_path).unwrap();

    let stdout_burst = measure_process(&invocation(
        &["streams", "131072", "0"],
        1024,
        Duration::from_secs(2),
    ))
    .expect("stdout burst stream capture measurement");
    assert_eq!(stdout_burst.status, MeasuredStatus::OutputLimit);
    assert!(stdout_burst.output_bytes > 1024);
    assert_eq!(stdout_burst.stderr_bytes, 0);
    let stdout_path = stdout_burst.stdout_path.as_deref().expect("saved stdout");
    let stderr_path = stdout_burst.stderr_path.as_deref().expect("saved stderr");
    assert!(std::fs::metadata(stdout_path).unwrap().len() <= 1024);
    assert!(std::fs::metadata(stderr_path).unwrap().len() <= 1024);
    std::fs::remove_file(stdout_path).unwrap();
    std::fs::remove_file(stderr_path).unwrap();

    let stderr_burst = measure_process(&invocation(
        &["streams", "0", "131072"],
        1024,
        Duration::from_secs(2),
    ))
    .expect("stderr burst stream capture measurement");
    assert_eq!(stderr_burst.status, MeasuredStatus::OutputLimit);
    assert_eq!(stderr_burst.output_bytes, 0);
    assert!(stderr_burst.stderr_bytes > 1024);
    let stdout_path = stderr_burst.stdout_path.as_deref().expect("saved stdout");
    let stderr_path = stderr_burst.stderr_path.as_deref().expect("saved stderr");
    assert!(std::fs::metadata(stdout_path).unwrap().len() <= 1024);
    assert!(std::fs::metadata(stderr_path).unwrap().len() <= 1024);
    std::fs::remove_file(stdout_path).unwrap();
    std::fs::remove_file(stderr_path).unwrap();
...
fn cancellation_terminates_and_retains_diagnostics_without_a_valid_sample() {
    ...
    let mut request = invocation(&["sleep", "10000"], 1024, Duration::from_secs(2));
    ...
    request.args = vec![
        "-c".to_owned(),
        "printf started > \"$1\"; exec sleep 10000".to_owned(),
    ...
    let (target_started, triggered_at) = trigger.join().unwrap();
    assert!(target_started, "target-start marker was not observed");
    assert!(
        triggered_at.elapsed() < Duration::from_secs(2),
        "cancellation did not finish within its bounded grace"
    );
