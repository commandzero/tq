...
const RELEASE_ENV: &str = "TQ_TEST_WORKER_RELEASE";
const DESCENDANT_ENV: &str = "TQ_TEST_WORKER_DESCENDANT";
...
fn run_fault_measure(mode: &str, args: &[&str]) -> (FaultMeasurement, Duration) {
    ...
    let release_path = if mode == "non-reading" {
        let release = tempfile::NamedTempFile::new().expect("worker release marker");
        let path = release.path().to_owned();
        fs::remove_file(&path).expect("remove initial release marker");
        Some(path)
    } else {
        None
    };
    ...
    if let Some(release_path) = &release_path {
        command.env(RELEASE_ENV, release_path);
    }
    if mode == "malformed-descendant" {
        command.env(DESCENDANT_ENV, "1");
    }
    let mut child = command.spawn().expect("spawn worker coordinator");
    ...
    measurement.release_path = release_path;
    (measurement, elapsed)
}
...
fn worker_fault_coordinator() {
    ...
    if env::var_os(DESCENDANT_ENV).is_some() {
        let Err(MeasureError::Collection { stdout_path, .. }) = &result else {
            panic!("malformed descendant worker did not retain its capture: {result:?}");
        };
        let pid = wait_for_descendant_pid(stdout_path, request.timeout);
        // A malformed reply can return while its cleanup executor is pending.
        // Keep this fixture's coordinator alive until that executor has
        // cleaned the descendant; exiting would discard the owning thread.
        wait_until_gone(pid);
    }
...
fn run_fault_helper(mode: &str) {
    if mode == "non-reading" {
        ...
        let release =
            PathBuf::from(env::var_os(RELEASE_ENV).expect("non-reading worker release marker"));
        let deadline = std::time::Instant::now() + Duration::from_secs(30);
        while fs::metadata(&release).is_err() && std::time::Instant::now() < deadline {
    ...
    if mode == "malformed-descendant" {
        ...
        let timeout = Duration::from_micros(
            request["timeout_micros"]
                .as_u64()
                .expect("worker target timeout"),
        );
        wait_for_descendant_pid(&request_path(&request, "stdout_path"), timeout);
    }
    if mode == "reply-delay" {
        thread::sleep(Duration::from_millis(150));
    }
...
fn wait_for_descendant_pid(path: &std::path::Path, timeout: Duration) -> i32 {
    ...
    loop {
        ...
        assert!(
            std::time::Instant::now() < deadline,
            "descendant did not publish its PID before the target timeout"
        );
        thread::sleep(Duration::from_millis(2));
    }
}
...
fn non_reading_worker_reaches_its_bounded_deadline() {
    ...
    let release_path = measurement
        .release_path
        .clone()
        .expect("non-reading worker release path");
    let release_guard = ReleaseOnDrop(release_path.clone());
    let worker_pid = measurement
        .worker_pid
        .expect("non-reading worker PID was recorded");
    assert!(
        process_is_alive(worker_pid),
        "bounded caller failure must retain the worker until cleanup control"
    );
    fs::write(&release_path, b"release").expect("release retained worker fixture");
    wait_until_gone(worker_pid);
    drop(release_guard);
    fs::remove_file(release_path).expect("remove worker release marker");
    cleanup_fault_measurement(&measurement);
}
