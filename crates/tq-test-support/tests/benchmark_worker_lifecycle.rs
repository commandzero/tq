...
//!
//! The outer tests deliberately kill only their isolated coordinator or worker
//! child. The parent test process owns the temporary evidence directory and
//! uses the worker's documented request protocol so it can inspect the
//! target's descendant capture after the helper coordinator is gone. Their
//! helper entry points remain ignored because the outer tests invoke them
