...
fn input_line_number_is_input_context_while_ambient_platform_data_remains_denied() {
    for query in ["input_line_number", "input_filename", "now"] {
        let command = tq_cli::parse_args_with_policy(
            ["-ijson", "-ojson", "-c", query],
            tq_cli::CapabilityPolicy {
                platform: false,
                ..tq_cli::CapabilityPolicy::default()
            },
        )
        .expect("embedded query with an explicit platform policy");
        let mut stdout = Vec::new();
        let mut stderr = Vec::new();
        let result = tq_cli::run_with_io(command, &mut b"1\n".as_slice(), &mut stdout, &mut stderr);
        if query == "input_line_number" {
            ...
            assert_eq!(status, tq_cli::ExitStatus::Runtime, "{query}");
            assert!(stdout.is_empty(), "{query}");
            assert!(
                String::from_utf8_lossy(&stderr).contains("capability policy"),
                "{query}: {}",
