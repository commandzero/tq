...
fn paths_filter_keeps_prior_results_before_a_later_error() {
    ...
    assert_eq!(stderr, b"tq: runtime error: later\n");
}

#[test]
fn path_keeps_prior_paths_before_a_later_error() {
    let (status, stdout, stderr) = execute("path(.[0],error(\"later\"))", "[1]\n");
    assert_eq!(status.unwrap(), ExitStatus::Runtime);
    assert_eq!(stdout, b"[0]\n");
    assert_eq!(stderr, b"tq: runtime error: later\n");
}
...
fn delpaths_keeps_prior_documents_before_a_later_error() {
    let (status, stdout, stderr) = execute("delpaths([[0]],error(\"later\"))", "[1,2]\n");
    assert_eq!(status.unwrap(), ExitStatus::Runtime);
