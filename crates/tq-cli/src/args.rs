...
const OPTION_REGISTRY: &[OptionSpec] = &[
    OptionSpec {
        ...
        value: true,
        description: "select automatic detection or an input format (see Formats below)",
    },
    OptionSpec {
        short: Some('o'),
        syntax: "-o, --output-format FORMAT",
        value: true,
        description: "select TOON (default) or another output format (see Formats below)",
    },
    OptionSpec {
        short: Some('n'),
        syntax: "-n, --null-input",
...
pub fn generated_help() -> String {
    let names = NativeFormat::ALL.map(|format| format.descriptor().name);
    let mut help = format!(
...
mod tests {
    ...
    #[test]
    fn help_summary_uses_the_complete_format_catalog() {
        let help = generated_help();
        let names = tq_formats::NativeFormat::ALL
