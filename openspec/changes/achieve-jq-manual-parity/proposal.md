...
- Preserve native TOON/YAML input extensions, TOON framing, and measured token-saving reports. Output representation is not a reason to weaken JSON-semantic correctness.
...
The baseline is commit `dcabecc` and its checked-in manual source inventory and comparison report. The proposal is planning only; it does not change code, accept new baselines, or claim compatibility before the gate passes.
