...
This is a historical supplemental evaluation for the Linux issue #30 smoke
...
`tq-json` pairs retain an automated `unavailable` status. Manual inspection is
supplemental evidence only: it does not establish machine-readable comparability
or promote these rows to accepted comparisons. The accepted denominator is six
smoke pairs, or 416 together with the 410 standard pairs. Unavailable rows stay
visible below, but their timing and RSS changes are not ranked or accepted.
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `format-base64-startup` / `tq-json` | 30 / 30 | 1.2 ms (MAD 0.0 ms) | 1.2 ms (MAD 0.0 ms) | unavailable | 7.9 MiB (sample MAD 0.1 MiB) | 7.8 MiB (sample MAD 0.0 MiB) | unavailable |
...
| `object-deep-merge` / `tq-json` | 30 / 30 | 48.5 ms (MAD 0.9 ms) | 48.4 ms (MAD 1.1 ms) | unavailable | 36.8 MiB (sample MAD 0.1 MiB) | 36.8 MiB (sample MAD 0.1 MiB) | unavailable |
...
| `startup` / `tq-json` | 30 / 30 | 1.0 ms (MAD 0.0 ms) | 1.0 ms (MAD 0.0 ms) | unavailable | 7.1 MiB (sample MAD 0.1 MiB) | 7.0 MiB (sample MAD 0.1 MiB) | unavailable |
...
No comparable row reaches the 20% disclosure threshold or the 50% blocking
threshold. The six comparable rows pass the issue #30 smoke regression policy.
The three unavailable rows do not receive a threshold verdict.
...
These observations remain supplemental evidence for three measured rows, not
accepted comparisons. Their original automated `unavailable` statuses remain
unchanged in the retained raw reports. The generator path-normalization fix is
implemented for future reports; it does not reclassify these immutable raw
artifacts. Acceptance would require separately validated machine-readable
comparability evidence or a new comparable campaign. Neither is supplied here.
