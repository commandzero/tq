...
### Requirement: Stack Overflow scenario review pages
The separately invoked Stack Overflow benchmark harness SHALL publish a summary at `docs/tests/stack-overflow/index.md` and one Markdown page per checked-in scenario. Each scenario page SHALL identify its source question, selected answer, exact benchmark query, and input. Adapted benchmark queries MUST NOT be presented as complete coverage of the original question. The harness SHALL generate a bottom `## Results` section with correctness outcomes and comparable tool metrics while preserving authored review text outside the generated section.

The harness SHALL support rendering saved measurements without executing benchmarks. Rendering MUST validate that the measurements belong to the selected scenarios and MUST retain unsuccessful outcomes. Historical measurements without authoritative RSS provenance MUST be labeled unverified rather than accepted as current benchmark evidence.

#### Scenario: Publish a Stack Overflow campaign
- **WHEN** the separately invoked campaign produces a report
- **THEN** the index summarizes coverage and links every scenario page, and each page shows the tools and their measured metrics together

#### Scenario: Regenerate reviewed results
- **WHEN** saved measurements are rendered into existing scenario pages
- **THEN** the harness replaces generated results, preserves authored review text, and does not run any measured tools

#### Scenario: Reject unrelated saved measurements
- **WHEN** a saved campaign does not match the selected scenario identities, queries, or inputs
- **THEN** rendering fails before publishing misleading scenario results

### Requirement: Stack Overflow output captures
The Stack Overflow harness SHALL capture each scenario's exact TOON stdout and
JSON comparison stdout separately from timed benchmark samples and SHALL
serialize the captures and their derived output metrics in the saved report for
render-only use. A normal run SHALL capture outputs. The `outputs --input SAVED_BENCHMARK.json --output
ENRICHED_REPORT.json --report-dir docs/tests/stack-overflow` command SHALL refresh
captures without taking new timing samples, retain benchmark provenance, and
record separate output-tool-capture provenance. Rendering SHALL consume the
saved report and MUST NOT invoke tools.

#### Scenario: Count exact captured stdout
- **WHEN** a scenario has a TOON capture and compact `jq -c` or expanded `jq` stdout that is a nonempty complete JSON value or ordered result stream with equivalent values, order, and cardinality
- **THEN** the report counts the exact stdout bytes including the final LF with `o200k_base` and `cl100k_base`, and reports `Diff = TOON - JSON` and `% = 100 × (TOON - JSON) / JSON`, with `%` keeping the sign of `Diff`, for each JSON form

#### Scenario: Exclude invalid JSON comparisons
- **WHEN** a compact or expanded JSON capture is empty, raw, non-JSON, exits with an error, or is not equivalent to the TOON values in order and cardinality
- **THEN** the report excludes that comparison and records the reason without adding `--seq` or formatting stdout to make it eligible

#### Scenario: Refresh captures without retiming
- **WHEN** `outputs` refreshes a saved Stack Overflow report
- **THEN** it invokes tools only for output capture, leaves existing benchmark timing and provenance intact, records output-tool-capture provenance separately, and writes the enriched report for a later tool-free render
...
Every authoritative benchmark run SHALL use the native accounting and direct-invocation requirements below, outside the sandbox with the required process-inspection permissions. `/usr/bin/time -l` on macOS and `/usr/bin/time -v` on Linux SHALL be used only for independent counter validation. Historical time-wrapper evidence SHALL remain labeled with its collection method and MUST NOT be compared with incompatible native-accounting measurements. Missing or invalid RSS SHALL invalidate the comparison and require a rerun; it MUST NOT count as passing memory validation.
...
- **WHEN** a run lacks authoritative native peak RSS because it ran without required elevated process-inspection permission
